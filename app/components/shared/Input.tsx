...
const Input = ({label, id, type, disabled, formatPrice, required, register, errors}:inputProps) => {
  return (
    <div className='w-full relative'>
      ...
      <input 
        ...
        placeholder=' '
        type={type}
        className={`peer w-full p-3 pt-6 font-light bg-white border-2 rounded-md transition disabled:cursor-not-allowed outline-none disabled:opacity-70 ${formatPrice ? 'pl-9' : 'pl-4'} ${errors[id] ? 'border-rose-500' : 'border-neutral-300'} ${errors[id] ? 'focus:border-rose-500' : 'focus:border-black'}`}
      />
